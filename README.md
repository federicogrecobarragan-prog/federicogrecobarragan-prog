...
| **Apps mobile · Play Store · iOS** | Framework completo Expo → Play Store (build, Hermes, Edge Function LLM, data safety, closed testing 12×14d, legales). Apps de referencia listas. | `mobile-playbook`, `folio-os`, `alimentando-a-popa`, `mishizen` |
...
| 🔒 [folio-os](https://github.com/federicogrecobarragan-prog/folio-os) | **Folio.OS** — app móvil de finanzas personales para Argentina (React Native + Expo, local-first). Dashboard caja+proyección, carga de gastos ARS/USD, objetivos de ahorro con aportes, USD Radar (dolarapi con histórico real), Contador IA (Edge Function → OpenRouter, contexto financiero completo), racha de carga, backup/export JSON+CSV. **En closed testing Play Store** (v1.1.0 en revisión). |
| 🔒 [mishizen](https://github.com/federicogrecobarragan-prog/mishizen) | **MishiZen** — app móvil de bienestar (React Native + Expo SDK 56, local-first). Meditaciones con paisajes sonoros reales, respiración guiada (orbe animado + inhalar/exhalar reales), hábitos con rachas, **diario de reflexión** (free 3/día · premium ilimitado), progreso/logros, despertar suave (notificaciones locales) y **Mishi IA** (gato calico compañero; cerebro local + seam Edge Function → OpenRouter free). Mascota anime, copy es-AR, modo oscuro. AAB con AD_ID/RECORD_AUDIO bloqueados. **En closed testing Play Store** (v1.0.0 aprobada, com.mishizen.app). |
...
| 🔒 [decretando-disney-web](https://github.com/federicogrecobarragan-prog/decretando-disney-web) | **Decretando Disney** — web de agencia de viajes (Martín & Mica): Disney, Universal, cruceros y Caribe. SPA estática (React 18 por CDN + JSX/Babel en navegador, sin build): aurora animada (canvas), **Mickey 3D interactivo** (model-viewer + GLB), 7 categorías de destinos, video-cards, testimonios, formulario `mailto:`. **En vivo:** [decretandodisney.com](https://decretandodisney.com) (Hostinger Premium, SSL). Deploy con auditoría pre-deploy multiagente (5 agentes, 0 blockers) + React prod/SRI + `.htaccess` MIME/cache + Open Graph. **Pipeline de assets local (reutilizable):** personaje riggeado en Mixamo → **Blender headless** (`fbx2glb.py`: FBX→GLB texturizado, multi-animación) + videos de destino comprimidos con **ffmpeg 2-pass** (155→19 MB sub-20). |
