...
| 🔒 [craneo-lrossi](https://github.com/federicogrecobarragan-prog/craneo-lrossi) | **CRÁNEO** — overlay del installer `genesis` para el cliente **Dos Hache (Lucas Rossi)**, agente operativo de planificación de rutas primera milla MercadoLibre. Branding/cliente vía `genesis.client.yaml` (runtime=hermes, connection=subscription CODEX). **Knowledge-pack reformulado a operativa logística** (reglas de oro, 7 etapas, 9 pilares, R0-R12, verificación 5 capas → con ejemplos de ruteo, choferes, asignaciones, Logistics y planilla "Colecta Buenos Aires"). **Mission-pack `lrossi-primera-milla`**: automatiza los pasos 3 (procesar texto de Logistics → tabla), 5 (asignar vehículo+chofer por 5 criterios) y 7 (mensajes WhatsApp) del proceso actual de 4hr/día → meta <2hr. Scripts checker (`verify_vps_hardening.sh`, `smoke_codex_wizard.py`, `rubric_doctrine.py`) + 4 playbooks `/dame` para deploy (Fase B). Fase A offline completa; Fase B en standby esperando credenciales VPS Hostinger + CODEX del cliente. Familia OdJ (cliente). |
