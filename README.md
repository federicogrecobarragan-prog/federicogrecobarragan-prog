...
| **Apps mobile · Play Store · iOS** | Framework completo Expo → Play Store (build, Hermes, Edge Function LLM, data safety, closed testing 12×14d, legales). Apps de referencia listas. | `mobile-playbook`, `folio-os`, `alimentando-a-popa` |
...
| 🔒 [folio-os](https://github.com/federicogrecobarragan-prog/folio-os) | **Folio.OS** — app móvil de finanzas personales para Argentina (React Native + Expo, local-first). Dashboard caja+proyección, carga de gastos ARS/USD, objetivos de ahorro con aportes, USD Radar (dolarapi), Contador IA (Edge Function → OpenRouter). En closed testing Play Store. |
...
| 🔒 [oficinadejhonson-web](https://github.com/federicogrecobarragan-prog/oficinadejhonson-web) | Sitio público **OFICINA DE JHONSON** — [www.oficinadejhonson.com](https://www.oficinadejhonson.com). HTML estático (Black Code design system) + widget de chat. Frontend deploy en Vercel; backend del chat en VPS. |
